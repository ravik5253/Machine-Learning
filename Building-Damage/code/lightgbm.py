...
data = pd.read_csv('./Dataset/useful_data_final.csv')
...
def break_num_features(data,num_feature):
    continues_feature = ['plinth_area_sq_ft','height_ft_diff','height_ft_pre_eq','height_ft_post_eq']
    binary_feature = [x for x in num_feature if x not in continues_feature and data[x].nunique()==2]
...
#data.drop(labels=['district_id','ward_id','vdcmun_id'],axis=1,inplace=True)
...
train_x = train_x.join(pd.get_dummies(train_y, prefix="pred").astype(int))
prior_0, prior_1, prior_2, prior_3, prior_4 = train_x[["pred_0.0", "pred_1.0", "pred_2.0","pred_3.0", "pred_4.0"]].mean()
attributes = product(("district_id", "vdcmun_id","ward_id"), zip(("pred_0.0","pred_1.0", "pred_2.0","pred_3.0", "pred_4.0"), (prior_0, prior_1, prior_2, prior_3, prior_4)))
attributes = product(("district_id"), zip(("pred_0.0", "pred_1.0"), (prior_0, prior_1)))

skf = StratifiedKFold(5,random_state=512)
for variable, (target, prior) in attributes:
    hcc_encode(train_x, test_x, variable, target, prior, k=5, r_k=None)
    for train, test in skf.split(np.zeros(len(train_x)), train_y):
        hcc_encode(train_x.iloc[train], train_x.iloc[test], variable, target, prior, k=5, r_k=0.01, update_df=train_x)


test_x.drop(['building_id','damage_grade'],inplace=True,axis=1)
dtrain_y = data[:len(train)]['damage_grade']
train_x.drop(['building_id','damage_grade',"pred_0.0", "pred_1.0", "pred_2.0","pred_3.0", "pred_4.0"],inplace=True,axis=1)


data_head = test_x.head(10)
train_test_head = train_x.head(10)
...
sel_features = ranking[ranking.Rank>3]['Features'].values

sel_features = [x for x in sel_features if xtrain[x].nunique()==2]

scaler = StandardScaler()
scaler.fit(xtrain[sel_features])
xtrain_s = scaler.transform(xtrain[sel_features])
xtest_s = scaler.transform(xtest[sel_features])
...
ica2_results_train = ica.fit_transform(xtrain_s)
ica2_results_test = ica.transform(xtest_s)

# GRP 15
grp = GaussianRandomProjection(n_components=n_comp, eps=0.1, random_state=420)
grp_results_train = grp.fit_transform(xtrain_s)
grp_results_test = grp.transform(xtest_s)

# FAG 23
fag = FeatureAgglomeration(n_clusters=n_comp, linkage='ward')
fag_results_train = fag.fit_transform(xtrain_s)
fag_results_test = fag.transform(xtest_s)
...
x=4
results_train = np.concatenate([ica2_results_train[:,:x],grp_results_train[:,:x],fag_results_train[:,:x]],axis=1)
results_test = np.concatenate([ica2_results_test[:,:x],grp_results_test[:,:x],fag_results_test[:,:x]],axis=1)
results_train = pd.DataFrame(results_train)
results_test = pd.DataFrame(results_test)
results_train.columns =  ['col'+str(x) for x in results_train.columns]
results_test.columns = results_train.columns

xtrain['lda1']=xtrain_s[:,0]
xtrain['lda2']=xtrain_s[:,1]

xtest['lda1']=xtest_s[:,0]
xtest['lda2']=xtest_s[:,1]


ranking = pd.read_csv('RFECV-ranking-final.csv')
not_sel_features = ranking[ranking.Rank>2]['Features'].values
predictors = list([x for x in xtrain.columns if x not in list(not_sel_features)+list(cat_feature)])
#predictors = [ x for x in predictors if x not in remove_features]
num_feature = [x for x in predictors if x not in cat_feature and x not in ['building_id', 'damage_grade']]
# break numerical feature into discrete,binary,continues
binary_feature,discrete_feature,continues_feature = break_num_features(xtrain,num_feature)
#predictors = [col for col in predictors if col not in col_su]
categorical_feature=[col for col in predictors if col in cat_feature or xtrain[col].nunique()==2]
# lightgbm
num_feature = num_feature+ ['lda1','lda2']


xtrain[predictors] = results_train[predictors]
xtest[predictors] = results_test[predictors]
...
predictors =  list(imp_df[imp_df.importance_split>90]['feature'] )  
...
yprob = predict.reshape(ytest.shape[0],5)
ylabel = np.argmax(predict, axis=1)
predictions = []
for x in predict:
...
rlgbm = lgb.LGBMClassifier(random_state=512,subsample=0.8,colsample_bytree=0.623,num_leaves=55,max_depth=7,learning_rate=0.2,boosting_type = "gbdt",objective ='multiclass',nthread = 4,verbose = 0)
rflgbm = RFECV(estimator=rlgbm,cv=StratifiedKFold(n_splits=5,random_state=512).split(train_x,train_y),step=4,scoring='f1_weighted',n_jobs=4,verbose=2)

start_time = timer(None)
rflgbm.fit(train_x,train_y)
timer(start_time)



print('\n Optimal number of features: %d' % rflgbm.n_features_)
sel_features = [f for f, s in zip(train_x.columns, rflgbm.support_) if s]
print('\n The selected features are {}:'.format(sel_features))

plt.figure(figsize=(12, 9))
plt.xlabel('Number of features tested x 4')
plt.ylabel('Cross-validation score (AUC)')
plt.plot(range(1, len(rflgbm.grid_scores_) + 1), rflgbm.grid_scores_)
plt.savefig('RFECV-final.png', dpi=150)
plt.show()


ranking = pd.DataFrame({'Features': train_x.columns})
ranking['Rank'] = np.asarray(rflgbm.ranking_)
ranking.sort_values('Rank', inplace=True)
ranking.to_csv('RFECV-ranking-final.csv', index=False)
