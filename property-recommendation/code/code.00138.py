...
properti = pd.read_csv('./Dataset/Property dataset/Properties.csv')
...
#        threshold_ind = indices
    ...
    indices = df[df.distance<19]['indices'].tolist()
...
cols = [x for x in accounts.columns if x not in ['id_accs','cluster']]
acc_neighbors = NearestNeighbors(n_neighbors=10,algorithm='ball_tree').fit(np.array(accounts[cols]))
cols = [x for x in properties.columns if x not in ['id_props','cluster']]
