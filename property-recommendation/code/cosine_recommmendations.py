...
class CosineBasedRecommender:
    
    MODEL_NAME = 'Cosine_Based'
    ...
    def cosine_get_top_prop(prop_id,topn):
        cols = [x for x in properties.columns if x not in ['id_props']]
        X = properties.loc[properties.id_props==prop_id][cols]   
        cosine_similarities = cosine_similarity(X,properties_df[cols])
        similar_indices = cosine_similarities.argsort().flatten()[-topn:]
        distance = -np.sort(-cosine_similarities)
        return distance,similar_indices
        
    def cosine_get_top_accs(acc_id,topn): 
        cols = [x for x in accounts.columns if x not in ['id_accs']]
        X = accounts.loc[accounts.id_accs==acc_id][cols]   
        cosine_similarities = cosine_similarity(X,accounts[cols])
        similar_indices = cosine_similarities.argsort().flatten()[-topn:]
        distance = -np.sort(-cosine_similarities)
        return distance,similar_indices
    ...
    def recommend_properties(accs_id):
        threshold_ind = self.cosine_get_top_accs(accs_id,5)
        ...
        for id_props in ids_props:
            indices = self.cosine_get_top_prop(id_props,3)
            df = df + list(indices)
        return properties_df['id_props'][df].unique().tolist()
    
    def get_recommendations():
        sub = pd.DataFrame(columns = ['id_accs','id_prop'])
        for accs in list(test['id_accs']):
            ids_props = self.recommend_properties(accs)
...
def cosine_get_top_accs(acc_id,topn): 
    cols = [x for x in accounts.columns if x not in ['id_accs']]
    X = accounts.loc[accounts.id_accs=='0012A000023XlWaQAK'][cols]   
    cosine_similarities = cosine_similarity(X,accounts[cols])
    cosine_similarities = pd.Series(cosine_similarities.flatten())
    similar_indices = cosine_similarities.argsort().flatten()
    cosine_similaritiesd = cosine_distances(X,accounts[cols])
    similar_indices = cosine_similarities.argsort().flatten()[-topn:]
    similar_indicesd = cosine_similaritiesd.argsort().flatten()[-topn:]
    df = pd.DataFrame({'distance':cosine_similarities.values,'indices':cosine_similarities.index})
    return df.sort_values(by = 'distance',ascending=False).top(topn)
     
res = df.sort_values(by = 'distance',ascending=False).head(10)
res1[::-1].sort()    
res2 = id(res1)

res1 = -np.sort(-res1)[:,:10]
...
def get_properties_recommended_for_account(accs_id):
    threshold_ind = cosine_get_top_accs(accs_id,5)
    similar_accounts = accounts["id_accs"][threshold_ind].tolist()
    ids_props = get_properties_purch_by_accs(similar_accounts)
    print('account ',accs_id,' is recommended ',len(ids_props),' properties purchased by',len(similar_accounts))
    df = []
    for id_props in ids_props:
        indices = cosine_get_top_prop(id_props,3)
        df = df + list(indices)
    return properties_df['id_props'][df].unique().tolist()
...
def get_recommendations():
    ...
    sub.to_csv('cosine_based.csv',index=False)
...
get_recommendations()
sub = pd.read_csv('cosine_based.csv')
res = sub.id_accs.value_counts()# -*- coding: utf-8 -*-
